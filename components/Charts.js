...
export function renderLineChartHtml(seriesList, labels) {
  ...
  const yAt = v => height - padY - (v / max) * (height - 2 * padY);

  const lines = seriesList.map(s => {
    const pts = s.data.map((v, i) => xAt(i) + ',' + yAt(v)).join(' ');
    const dots = s.data.map((v, i) => '<circle cx="' + xAt(i) + '" cy="' + yAt(v) + '" r="3" fill="' + s.color + '"/>').join('');
  ...
  const xLabels = labels.map((l, i) =>
    '<text x="' + xAt(i) + '" y="' + (height - 2) + '" text-anchor="middle" font-size="9" fill="var(--text-3)">' + l + '</text>'
  ).join('');
